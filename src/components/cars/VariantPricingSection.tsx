    ...
    <section className="py-10 md:py-20 bg-[#0a0a0a] border-t border-white/5 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-8 max-w-6xl relative z-10">
        ...
        <div className="text-center mb-6 md:mb-12">
          <h2 className="text-lg md:text-3xl font-black uppercase tracking-tight text-white">
        ...
        <div className="relative group/nav">
          ...
          {allVariants.length > 1 && (
            <>
              <button 
                onClick={() => {
                  const newIndex = activeDot > 0 ? activeDot - 1 : allVariants.length - 1;
                  scrollTo(newIndex);
                }}
                className="absolute left-0 lg:left-[-40px] top-[140px] md:top-1/2 -translate-y-1/2 z-40 w-10 h-10 flex items-center justify-center rounded-full border border-white/10 bg-black/40 text-white/50 hover:bg-primary hover:text-white transition-all backdrop-blur-md opacity-0 group-hover/nav:opacity-100 shadow-2xl"
              >
                <ChevronLeft className="w-5 h-5" />
              </button>
              <button 
                onClick={() => {
                  const newIndex = activeDot < allVariants.length - 1 ? activeDot + 1 : 0;
                  scrollTo(newIndex);
                }}
                className="absolute right-0 lg:right-[-40px] top-[140px] md:top-1/2 -translate-y-1/2 z-40 w-10 h-10 flex items-center justify-center rounded-full border border-white/10 bg-black/40 text-white/50 hover:bg-primary hover:text-white transition-all backdrop-blur-md opacity-0 group-hover/nav:opacity-100 shadow-2xl"
              >
          ...
          {/* Draggable Row (Native Scroll) */}
          <div 
            ref={scrollRef}
            className="flex overflow-x-auto no-scrollbar pb-4 select-none"
            style={{ 
          ...
          >
            ...
            {allVariants.map((v, i) => {
              ...
              const isCenter = activeDot === i;
              
              return (
                <div 
                  key={i} 
                  className="w-full flex-shrink-0 snap-center outline-none py-4 px-2"
                >
                    
                    {/* Car Image with cinematic effects */}
                    <div className="relative w-full max-w-xl mx-auto" style={{ height: "clamp(160px, 25vw, 280px)" }}>
                      <div className={`absolute bottom-[2%] left-1/2 -translate-x-1/2 w-64 md:w-80 h-12 md:h-16 bg-primary/10 rounded-full blur-[30px] md:blur-[40px] z-0 transition-opacity duration-700 ${isCenter ? 'opacity-100' : 'opacity-0'}`} />
                      ...
                      <div className="relative w-full h-full z-10 px-4">
                        <Image
                          ...
                          alt={v.name}
                          fill
                          className="object-contain object-center drop-shadow-[0_15px_35px_rgba(0,0,0,1)] pointer-events-none"
                          draggable={false}
                      ...
                      <div className={`absolute bottom-[-15%] left-0 w-full h-[40%] opacity-[0.2] scale-y-[-1] pointer-events-none transition-opacity duration-700 ${isCenter ? 'opacity-[0.2]' : 'opacity-0'}`}>
                        <Image src={v.image || carImage} alt="" fill className="object-contain blur-[4px] mask-gradient-to-t" />
                      </div>
                    </div>

                    {/* Info Row */}
                    <div className={`mt-8 md:mt-12 flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-0 max-w-4xl mx-auto px-4 md:px-0 transition-opacity duration-500 ${isCenter ? 'opacity-100' : 'opacity-20'}`}>
                      {/* Left — Name & Price */}
                      <div className="flex-1 w-full md:pr-12 md:border-r border-white/10 flex flex-col md:items-end text-center md:text-right">
                        ...
                        <div className="flex flex-col md:items-end">
                          ...
                          <p className="text-3xl md:text-5xl font-black text-white leading-none mb-1 drop-shadow-[0_0_15px_rgba(204,0,0,0.3)]">
                            {v.price}
                          </p>
